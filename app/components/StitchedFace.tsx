...
interface StitchedGridProps {
  ...
  offsetZ?: number;
  stitchEdges?: { top?: number; right?: number; bottom?: number; left?: number }; // Ratio: 1=same, 2=half, etc.
...
const createStitchedGeometry = (
  ...
  if (topR > 1) {
    for (let x = 2; x < N - 2; x += 2) {
      ...
      addTri(D, A, E); addTri(A, C, E); addTri(E, C, F);
    }
  } else {
    // Standard Top Edge
    for (let x = 1; x < N - 1; x++) {
      ...
      const a = getIdx(x, 0), b = getIdx(x + 1, 0), c = getIdx(x + 1, 1), d = getIdx(x, 1);
      addTri(a, d, b); addTri(b, d, c);
    }
  }
  ...
  if (botR > 1) {
    for (let x = 2; x < N - 2; x += 2) {
       ...
       addTri(D, A, E); addTri(E, A, C); addTri(F, C, E);
    }
  } else {
    for (let x = 2; x < N - 2; x++) {
      const a = getIdx(x, N - 1), b = getIdx(x + 1, N - 1), c = getIdx(x + 1, N), d = getIdx(x, N);
      addTri(a, d, b); addTri(b, d, c);
    }
  }
  ...
  if (leftR > 1) {
    ...
    for (let z = 2; z < N - 2; z++) {
      const a = getIdx(0, z), b = getIdx(1, z), c = getIdx(1, z + 1), d = getIdx(0, z + 1);
      addTri(a, d, b); addTri(b, d, c);
    }
  }
  ...
  if (rightR > 1) {
    for (let z = 2; z < N - 2; z += 2) {
      ...
      addTri(D, E, A); addTri(E, C, A); addTri(E, F, C);
    }
  } else {
    for (let z = 2; z < N - 2; z++) {
      const a = getIdx(N - 1, z), b = getIdx(N, z), c = getIdx(N, z + 1), d = getIdx(N - 1, z + 1);
      addTri(a, d, b); addTri(b, d, c);
    }
  }
  ...
  // Helper for Corners
  const addCorner = (xBase: number, zBase: number, stitchH: number, stitchV: number) => {
    ...
    // Case 2: None Stitched (1:1) -> Standard 2x2 grid
      ...
      for (let dz = 0; dz < 2; dz++) {
        for (let dx = 0; dx < 2; dx++) {
          const a = i(dx, dz), b = i(dx+1, dz), c = i(dx+1, dz+1), d = i(dx, dz+1);
          addTri(a, d, b); addTri(b, d, c);
        }
      }
    }
    // Case 3: Mixed (Top Stitched, Left Standard)
      ...
      {
         const a=i(0,1), b=i(1,1), c=i(1,2), d=i(0,2);
         addTri(a,d,b); addTri(b,d,c);
      }
      ...
      {
         const a=i(1,1), b=i(2,1), c=i(2,2), d=i(1,2);
         addTri(a,d,b); addTri(b,d,c);
      }
    }
    // Case 4: Mixed (Top Standard, Left Stitched)
      ...
      // TR Quad: 10-20-21-11
      {
        const a=i(1,0), b=i(2,0), c=i(2,1), d=i(1,1);
        addTri(a,d,b); addTri(b,d,c);
      }
      // BR Quad: 11-21-22-12
      {
  ...
  // TR (Top-Right)
  {
    ...
    if (sH > 1 && sV > 1) {
    ...
    else if (sH === 1 && sV === 1) { // Standard
      for(let dz=0; dz<2; dz++) for(let dx=0; dx<2; dx++) {
        const a=i(dx,dz), b=i(dx+1,dz), c=i(dx+1,dz+1), d=i(dx,dz+1);
        addTri(a,d,b); addTri(b,d,c);
      }
    }
    else if (sH > 1 && sV === 1) { // Top Stitch, Right Standard
      ...
      addTri(i(0,0), i(0,1), i(1,1));
      // BL Quad
      { const a=i(0,1),b=i(1,1),c=i(1,2),d=i(0,2); addTri(a,d,b); addTri(b,d,c); }
      // BR Quad
      { const a=i(1,1),b=i(2,1),c=i(2,2),d=i(1,2); addTri(a,d,b); addTri(b,d,c); }
    }
    ...
    // For "Mixed" (Top Standard, Right Stitched)
       ...
       // TL Quad
       { const a=i(0,0),b=i(1,0),c=i(1,1),d=i(0,1); addTri(a,d,b); addTri(b,d,c); }
       // BL Quad
       { const a=i(0,1),b=i(1,1),c=i(1,2),d=i(0,2); addTri(a,d,b); addTri(b,d,c); }
    }
  }
  ...
  // BL (Bottom-Left)
  {
    ...
    // Mixed cases omitted for brevity - relying on consistent 2:1 for now
    ...
    else if (sH > 1 && sV === 1) { // Bot Stitch, Left Std
       ...
       { const a=i(0,0),b=i(1,0),c=i(1,1),d=i(0,1); addTri(a,d,b); addTri(b,d,c); }
       // TR
       { const a=i(1,0),b=i(2,0),c=i(2,1),d=i(1,1); addTri(a,d,b); addTri(b,d,c); }
    }
    else if (sH === 1 && sV > 1) { // Bot Std, Left Stitch
  ...
  // BR (Bottom-Right)
  {
    ...
    else if (sH > 1 && sV === 1) { // Bot Stitch, Right Std
    ...
    else if (sH === 1 && sV > 1) { // Bot Std, Right Stitch
       // Right Stitch: i(2,0)-i(2,2)-i(1,1)
       addTri(i(2,0), i(2,2), i(1,1));
       // Top Filler: i(1,0)-i(2,0)-i(1,1)
       addTri(i(1,0), i(2,0), i(1,1));
...
function StitchedFace(props: StitchedGridProps) {
  ...
  }, [props.segments, props.face, props.radius, props.scale, props.offsetX, props.offsetZ, props.stitchEdges, props.isHole, props.holeSize]);
