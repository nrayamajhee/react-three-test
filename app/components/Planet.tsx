  ...
  const geometry = useMemo(() => {
    ...
    for (const faceIndices of ICOSAHEDRON_FACES) {
      ...
      for (let i = 0; i < baseSub; i++) {
        for (let j = 0; j <= i; j++) {
          const processSubFace = (v1: THREE.Vector3, v2: THREE.Vector3, v3: THREE.Vector3) => {
            ...
            for (let r = 0; r < k; r++) {
              for (let c = 0; c < r; c++) {
                ...
                if (i1 !== i3 && i3 !== i4 && i4 !== i1) indices.push(i1, i3, i4);
              }
          ...
          const v1 = slerp(slerp(A, B, i / baseSub), slerp(A, C, i / baseSub), (i === 0) ? 0 : j / i);
          const v2 = slerp(slerp(A, B, (i + 1) / baseSub), slerp(A, C, (i + 1) / baseSub), j / (i + 1));
          const v3 = slerp(slerp(A, B, (i + 1) / baseSub), slerp(A, C, (i + 1) / baseSub), (j + 1) / (i + 1));
          processSubFace(v1, v2, v3);

          if (j < i) {
            const vd1 = v1;
            const vd2 = v3;
            const vd3 = slerp(slerp(A, B, i / baseSub), slerp(A, C, i / baseSub), (j + 1) / i);
